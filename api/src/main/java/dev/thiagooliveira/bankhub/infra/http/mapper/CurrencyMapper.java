...
public interface CurrencyMapper {
  ...
  default dev.thiagooliveira.bankhub.http.dto.Currency map(Currency currency) {
    if (currency == null) {
      return null;
    }

    dev.thiagooliveira.bankhub.http.dto.Currency dto =
        new dev.thiagooliveira.bankhub.http.dto.Currency();
    dto.setName(currency.name()); // enum name: EUR, BRL
    dto.setSymbol(currency.symbol()); // symbol: €, R$
    return dto;
  }

  default Currency map(dev.thiagooliveira.bankhub.http.dto.Currency dto) {
